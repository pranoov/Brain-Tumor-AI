...
app = Flask(__name__)
app.secret_key = 'your-secret-key-here'  # Needed for session support
...
def home():
    if request.method == "POST":
        ...
        if imageFile and imageFile.filename:
            ...
            nextStep = nextSteps.get(prediction, "No next steps available.")
            # Pass image as a data URL for HTML <img> tag
            image_url = f"data:image/jpeg;base64,{newImg}"
            return render_template("results.html", content=prediction, explanation=explanation, image=image_url, nextSteps=nextStep)

        # If no image, show not found
        return render_template("results.html", content="not found!", explanation="No explanation available.", image="", nextSteps="No next steps available.")
    else:
...
@app.route("/results")
def result():
    # If user visits /results directly, show a default message
    return render_template("results.html", content="No result provided", explanation="No explanation available.", image="", nextSteps="No next steps available.")
